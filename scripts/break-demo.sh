...
#
# Usage:
#   ./break-demo.sh badimage   # ImagePullBackOff (default)
...
case "$MODE" in
  ...
  oom)
    ...
    echo "Dropped memory limit to 8Mi -> expect OOMKilled restarts."
    ;;
  *)
    echo "Unknown mode: $MODE (expected badimage|scaledown|oom)" >&2
    exit 1
    ;;
esac
